<?php session_start(); ?>
<html>

<head>
    <script src="https://code.jquery.com/jquery-3.6.0.js"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.3/font/bootstrap-icons.css">
    <link rel="stylesheet" href="style.css">
    <script>
    $(document).ready(function() {
      let pwd, cpwd;
      $("#pwd").keyup(function() {
        pwd = $("#pwd").val();
        var c_pwd = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{6,}$/;
        r_pwd = c_pwd.test(pwd);
        if (!r_pwd) {
          $("#submit").attr('disabled', true);
          $("#pwd_error").text("Enter Valid Password");
        } else {
          $("#pwd_error").text("");
          valid();
        }
      })
      $("#cpwd").keyup(function() {
        cpwd = $("#cpwd").val();
        var c_pwd = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{6,}$/;
        cr_pwd = c_pwd.test(cpwd);

        if (pwd != cpwd) {
          $("#submit").attr('disabled', true);
          $("#cp_error").text("Password does not match");
        } else {
          $("#cp_error").text("");
          valid()
        }
      })
      function valid() {
        if (r_pwd == true && cr_pwd==true) {
          $("#submit").attr('disabled', false);
        }
      }
    })
  </script>
</head>
<section class="vh-100">
    <div class="container-fluid h-custom">
        <div class="row d-flex justify-content-center align-items-center h-100">
            <div class="col-md-9 col-lg-6 col-xl-5">
                <img src="logo\logo2.png" class="img-fluid" alt="Sample image">
            </div>
            <div class="col-md-8 col-lg-6 col-xl-4 offset-xl-1">
                <form action="" method="post">
                    <h1>Enter Password</h1>

                    <div class="divider d-flex align-items-center my-4">
                    </div>

                    <!-- Email input -->
                    <div class="form-outline mb-4">
                        <input type="password"  name="pss" class="form-control form-control-lg" placeholder="Enter a valid Password"  id="pwd" />
                        <p id="pwd_error"></p>
                        <label class="form-label" for="pwd">Enter Password</label>
                    </div>
                    <div class="form-outline mb-4">
                        <input type="password"  name="cnfm" class="form-control form-control-lg" placeholder="Enter a valid Password" id="cpwd" />
                        <label class="form-label" for="cpwd">confirm password</label>
                        <p id="cp_error"></p>
                    </div>
                    <div class="text-center text-lg-start mt-4 pt-2">
                        <button type="submit" name="sub" class="btn btn-outline-dark btn-lg" id="submit">Change</button>
                        <p class="small fw-bold mt-2 pt-1 mb-0">Don't have an account? <a href="register.php" class="link-danger">Register</a></p>
                    </div>

                </form>
            </div>
        </div>
    </div>
    <div class="d-flex flex-column flex-md-row text-center text-md-start justify-content-between py-4 px-4 px-xl-5 bg-dark">
        <!-- Copyright -->
        <div class="text-white mb-3 mb-md-0">
            Copyright © 2020. All rights reserved.
        </div>
        <!-- Copyright -->

        <!-- Right -->
        <div>
            <!-- <a href="#!" class="text-white me-4">
                <i class="fab fa-facebook-f">fb</i>
            </a>
            <a href="#!" class="text-white me-4">
                <i class="fab fa-twitter">tw</i>
            </a>
            <a href="#!" class="text-white me-4">
                <i class="fab fa-google">gog</i>
            </a>
            <a href="#!" class="text-white">
                <i class="fab fa-linkedin-in">ln</i>
            </a> -->
        </div>
        <!-- Right -->
    </div>
</section>

</html>
<?php
if (isset($_POST["sub"]))
 {
  include("connection.php");

    $id = base64_decode(urldecode($_GET["id"]));
    $pass = $_POST["cnfm"];
    $query2= "UPDATE `user_table` SET`user_password`='$pass' WHERE `user_id` = '$id'";
    $ew=mysqli_query($con,$query2);
    if($ew)
{    
  echo"<script>alert('successfully updated');</script>";

}
 }
?>